...
static int moh_files_generator(struct ast_channel *chan, void *data, int len, int samples)
{
	...
	ast_debug(6, "moh_files_generator start\n");
...
static void *moh_files_alloc(struct ast_channel *chan, void *params)
{
	...
	ast_debug(6, "moh_files_alloc start\n");

	if (!chan->music_state && (state = ast_calloc(1, sizeof(*state)))) {
		ast_debug(6, "!chan->music_state && (state = ast_calloc(1, sizeof(*state))) \n");
		ast_mutex_init(&state->lock);
	...
	if (state->save_total != class->total_files || strcmp(state->name, class->name) != 0) {
		...
		}else if (ast_test_flag(class, MOH_SORTALPHA)) {
			ast_debug(6, "pos: %d\n", state->pos);
			state->pos = class->total_files - 1;
		}
	}
...
static void local_ast_moh_cleanup(struct ast_channel *chan)
{
	...
	ast_debug(6, "local_ast_moh_cleanup start\n");
