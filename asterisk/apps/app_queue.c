...
static int forceautopause = 0;

static int pause_needs_login = 0;

/*! \brief queues.conf [general] option */
static int firstperiodicannounce = 0;
...
static int say_position(struct queue_ent *qe, int ringing)
{
	...
	ast_debug(6, "say_position start\n");
    ...
    if ((avgholdmins+avgholdsecs) > 0 && qe->parent->announceholdtime &&
		...
		if (avgholdsecs >= 1) {
			...
			if (res)
				goto playout;
			if (!strcasecmp(qe->chan->language, "ja")) {
				res = play_file(qe->chan, "desu");
				ast_debug(6, "Value: desu\n");
...
static void queue_set_global_params(struct ast_config *cfg)
{
	...
	ast_debug(6, "queue_set_global_params start\n");
	
	/* Initialize global settings */
	forceautopause = 0;
	if ((general_val = ast_variable_retrieve(cfg, "general", "forceautopause"))){
		forceautopause = ast_true(general_val);
		ast_debug(6, "forceautopause: %d\n", forceautopause);
	}
	/* Initialize global settings */
	pause_needs_login = 0;
	if ((general_val = ast_variable_retrieve(cfg, "general", "pauseneedslogin"))){
		pause_needs_login = ast_true(general_val);
		ast_debug(6, "forceautopause: %d\n", pause_needs_login);
	}
	firstperiodicannounce = 0;
	if ((general_val = ast_variable_retrieve(cfg, "general", "firstperiodicannounce"))){
		firstperiodicannounce = ast_true(general_val);
		ast_debug(6, "forceautopause: %d\n", firstperiodicannounce);
	}
...
/*! \brief  reload_realtime_queues: reload realtime queues */
static void reload_realtime_queues(void)
{
	char *category;
	struct ast_config *config = NULL;

	ast_debug(6, "reload_realtime_queues start\n");
	
	config = ast_load_realtime_multientry("queues", "name !=", " ", SENTINEL);
	if (!config){
		ast_debug(6, "Value is NULL.\n");
		return;
	}

	category = ast_category_browse(config, NULL);
	while (category) {
		if (!load_realtime_queue(category))
			ast_log(LOG_WARNING, "Failed to load realtime queue %s.", category);
		category = ast_category_browse(config, category);
	}
...
static int reload_queues(int reload, struct ast_flags *mask, const char *queuename)
{
	...
	ast_debug(6, "reload_queues start\n");
