...
static int ast_say_enumeration_full_ja(struct ast_channel *chan, int num, const char *ints, const char *language, int audiofd, int ctrlfd);
...
static int ast_say_datetime_from_now_ja(struct ast_channel *chan, time_t t, const char *ints, const char *lang);
...
{
	ast_debug(6, "say_number_full start\n");
...
/*! \brief  ast_say_number_full_ja: Japanese syntax */
/* This is the default syntax, if no other syntax defined in this file is used */
static int ast_say_number_full_ja(struct ast_channel *chan, int num, const char *ints, const char *language, int audiofd, int ctrlfd)
{
	int res = 0;
	...
	ast_debug(6, "ast_say_number_full_ja start\n");
	...
	while (!res && num) {
		if (num < 0) {
			...
			if (num < 1000){ /* HYAKU */
				...
				if (num < 10000) { /* SEN */
					snprintf(fn, sizeof(fn), "digits/%d", (num /1000) * 1000);
         			num -= ((num / 1000) * 1000);
         		} else {
					if (num < 1000000) {  /* MAN */
						snprintf(fn, sizeof(fn), "digits/%d", (num/10000));
						res = ast_say_number_full_ja(chan, num / 10000, ints, language, audiofd, ctrlfd);
						if (res)
							return res;
						num %= 1000;
						snprintf(fn, sizeof(fn), "digits/thousand");
					} else {
						if (num < 1000000000) {	 /* OKU */
                  			res = ast_say_number_full_ja(chan, num / 100000000, ints, language, audiofd, ctrlfd);
							if (res)
								return res;
							num %= 1000000;
							ast_copy_string(fn, "digits/million", sizeof(fn));
...
/*! \brief  say_enumeration_full: call language-specific functions */
/* Called from AGI */
static int say_enumeration_full(struct ast_channel *chan, int num, const char *ints, const char *language, const char *options, int audiofd, int ctrlfd)
{
	ast_debug(6, "say_enumeration_full start\n");
	...
	if (!strncasecmp(language, "en", 2)) {        /* English syntax */
	   ...
	   return ast_say_enumeration_full_de(chan, num, ints, language, options, audiofd, ctrlfd);
		...
		ast_debug(6, "Language is %s.\n",language);
		return ast_say_enumeration_full_ja(chan, num, ints, language, audiofd, ctrlfd);
	}
...
/*! \brief  ast_say_enumeration_full_ja: Japanese syntax */
/* This is the default syntax, if no other syntax defined in this file is used */
static int ast_say_enumeration_full_ja(struct ast_channel *chan, int num, const char *ints, const char *language, int audiofd, int ctrlfd)
{
	int res = 0;
	...
	ast_debug(6, "ast_say_enumeration_full_ja start\n");
	...
	while (!res && num) {
		if (num < 0) {
			...
			res = ast_say_number_full_ja(chan, num, ints, language, audiofd, ctrlfd);
			if (!res)
				res = wait_file(chan, ints, "digits/banme", language);
			break;
		}
...
/*! \brief  ast_say_date_ja: Japanese syntax */
int ast_say_date_ja(struct ast_channel *chan, time_t t, const char *ints, const char *lang)
{
	...
	ast_debug(6, "ast_say_date_ja start\n");
	...
	if (!res) {
		snprintf(fn, sizeof(fn), "digits/d-%d", tm.tm_mday);
		res = ast_streamfile(chan, fn, lang);
		ast_debug(6, "Mday: %d\n", tm.tm_mday);
		if (!res){
	    	res = ast_waitstream(chan, ints);
...
{
	ast_debug(6, "say_date_with_format start\n");
...
/*! \brief  ast_say_time_ja: Japanese syntax */
int ast_say_time_ja(struct ast_channel *chan, time_t t, const char *ints, const char *lang)
{
	struct timeval when = { t, 0 };
	struct ast_tm tm;
	//struct tm tm;
	...
	ast_debug(6, "ast_say_time_ja start\n");
	...
	if (!hour){
		...
		hour -= 12;
		pm = 1;
		ast_debug(6, "hour: %d pm: %d\n", hour, pm);
	}
	if (pm) {
		if (!res){
			res = ast_streamfile(chan, "digits/p-m", lang);
			ast_debug(6, "Value: digits/p-m\n");
		}
	} else {
		if (!res){
			res = ast_streamfile(chan, "digits/a-m", lang);
			ast_debug(6, "Value: digits/a-m\n");
		}
	}
	if (!res){
		res = ast_waitstream(chan, ints);
	...
	if (!res){
		res = ast_say_number(chan, tm.tm_min, ints, lang, (char *) NULL);
		ast_debug(6, "tm_min: %d ints: %s\n", tm.tm_min, ints);
	}
	if (!res){
		res = ast_streamfile(chan, "digits/fun", lang);
		ast_debug(6, "Value: digits/fun\n");
	}
	if (!res){
		res = ast_waitstream(chan, ints);
		ast_debug(6, "ints: %s\n", ints);
...
{
	ast_debug(6, "say_datetime start\n");
...
/*! \brief  ast_say_datetime_ja: Japanese syntax */
int ast_say_datetime_ja(struct ast_channel *chan, time_t t, const char *ints, const char *lang)
{
	...
	ast_debug(6, "ast_say_datetime_ja start\n");
	
	ast_localtime(&when, &tm, NULL);
	//localtime_r(&t,&tm);
	if (!res){
		res = ast_say_number(chan, tm.tm_year + 1900, ints, lang, (char *) NULL);
		ast_debug(6, "tm_year: %d\n", tm.tm_year + 1900);
	}
	if (!res){
		res = wait_file(chan, ints, "digits/nen", lang);
		ast_debug(6, "Value: digits/nen\n");
...
{
	ast_debug(6, "say_datetime_from_now start\n");
	...
	if (!strncasecmp(lang, "en", 2)) {        /* English syntax */
		...
		ast_debug(6, "Language is %s.\n",lang);
		return ast_say_datetime_from_now_ja(chan, t, ints, lang);
	}
...
/*! \brief  ast_say_datetime_from_now_ja: Japanese syntax */
int ast_say_datetime_from_now_ja(struct ast_channel *chan, time_t t, const char *ints, const char *lang)
{
	struct timeval nowt = ast_tvnow(), when = { t, 0 };
	int res=0;
	int daydiff;
	...
	ast_debug(6, "ast_say_datetime_from_now_ja start\n");
...
/*! \brief  ast_say_date_with_format_ja: Japanese syntax */
int ast_say_date_with_format_ja(struct ast_channel *chan, time_t t, const char *ints, const char *lang, const char *format, const char *tzone)
{
	...
	ast_debug(6, "ast_say_date_with_format_ja start\n");
		...
		switch (format[offset]) {
			...
			case 'e':
				/* 1-31 nichi */
			        ...
			        snprintf(nextmsg,sizeof(nextmsg), "digits/d-%d", tm.tm_mday);
				res = wait_file(chan,ints,nextmsg,lang);
			...
			case 'Y':
				/* Year */
				if (tm.tm_year > 99) {
					res = wait_file(chan,ints, "digits/2000",lang);
					if (tm.tm_year > 100) {
						if (!res) {
							/* This works until the end of 2020 */
							snprintf(nextmsg,sizeof(nextmsg), "digits/%d", tm.tm_year - 100);
					...
					res = wait_file(chan,ints, "digits/1000",lang);
					if(!res)
					  res = wait_file(chan,ints, "digits/900",lang);
					if (!res) {
						if (tm.tm_year <= 20) {
							/* 1900 - 1920 */
							snprintf(nextmsg,sizeof(nextmsg), "digits/%d", tm.tm_year);
				...
				if (!res) res = wait_file(chan,ints, "digits/nen",lang);
				break;
			case 'P':
			case 'p':
				/* AM/PM */
			...
			case 'M':
				...
				if (!res) res = wait_file(chan,ints, "digits/fun",lang);
				break;
			case 'Q':
				/* Shorthand for "Today", "Yesterday", or ABdY */
				/* XXX As emphasized elsewhere, this should the native way in your
				 ...
				 * upon how recent the date is. XXX */
				{
					...
					if (beg_today < t) {
						...
						/* Within the last week */
						res = ast_say_date_with_format_ja(chan, t, ints, lang, "A", tzone);
					} else if (beg_today - 2628000 < t) {
						/* Less than a month ago - "Sunday, October third" */
						res = ast_say_date_with_format_ja(chan, t, ints, lang, "ABd", tzone);
					} else if (beg_today - 15768000 < t) {
						/* Less than 6 months ago - "August seventh" */
						res = ast_say_date_with_format_ja(chan, t, ints, lang, "Bd", tzone);
					} else {
						/* More than 6 months ago - "April nineteenth two thousand three" */
						res = ast_say_date_with_format_ja(chan, t, ints, lang, "BdY", tzone);
					}
				}
				break;
			case 'q':
