...
#define DEFAULT_PARK_TIME							600000	/*!< ms */
...
static struct parkeduser *park_space_reserve(struct ast_channel *park_me, struct ast_channel *parker, struct ast_park_call_args *args)
{
	...
	int ext = 0;
	const char *parkext_slot;
	const char *parkext;
	char hint[80];

	ast_debug(6, "park_space_reserve start\n");

	if (args->parkinglot) {
		ast_debug(6, "park_space_reserve 1\n");
		parkinglot = parkinglot_addref(args->parkinglot);
		parkinglotname = parkinglot->name;
		ast_debug(6, "park_space_reserve 1 parkinglotname = [%s]\n",parkinglotname);
	} else {
		ast_debug(6, "park_space_reserve 2\n");
		if (parker) {
			ast_debug(6, "park_space_reserve 2\n");
			parkinglotname = findparkinglotname(parker);
			ast_debug(6, "park_space_reserve 2 parkinglotname = [%s]\n",parkinglotname);
		} else { /* parker was NULL, check park_me (ParkAndAnnounce / res_agi) */
			ast_debug(6, "park_space_reserve 3\n");
			parkinglotname = findparkinglotname(park_me);
			ast_debug(6, "park_space_reserve 3 parkinglotname = [%s]\n",parkinglotname);
		}
		if (!ast_strlen_zero(parkinglotname)) {
			ast_debug(6, "park_space_reserve 4\n");
			parkinglot = find_parkinglot(parkinglotname);
			ast_debug(6, "park_space_reserve 3 parkinglot = [%s]\n",parkinglot->name);
		} else {
			...
			ast_debug(6, "park_space_reserve 5 parkinglot = [%s]\n",parkinglot->name);
		}
	}
	...
	if(ast_strlen_zero(parkingexten)){
		parkext_slot = pbx_builtin_getvar_helper(parker, "PARKCALLSLOT");
		ast_debug(6,"parkext_slot=%s\n" , parkext_slot);
		if (!parkext_slot || (ext = atoi(parkext_slot)) <= 0) {
			parkext = pbx_builtin_getvar_helper(parker, "PARKCALLEXT");
			ast_debug(6,"parkext=%s\n" , parkext);
			if (parkext) {
				parkingexten = parkext;
				if (ast_get_hint(hint, sizeof(hint), NULL, 0, parker, "default", parkext) &&
					sscanf(hint, "Local/%d@parkedcalls", &ext) == 1) {
					ext = (ext > parkinglot->cfg.parking_stop) ? 0 : ext - parkinglot->cfg.parking_start + 1;
					ast_debug(6,"ext=%d\n" , ext);
				}
			}
		} else {
			parkingexten = parkext_slot;
		}
	}

	ast_log(LOG_NOTICE,"parkingexten=%s\n" , parkingexten);
	if (!ast_strlen_zero(parkingexten)) {
		/*!
...
static int park_call_full(struct ast_channel *chan, struct ast_channel *peer, struct ast_park_call_args *args)
{
	...
	ast_debug(6, "park_call_full start\n");
	...
	if (peer && !ast_test_flag(args, AST_PARK_OPT_SILENCE)
		...
		parkext = atoi(pu->parkinglot->cfg.parkext);
		
		ast_say_digits(peer, pu->parkingnum - parkext, "", peer->language);
	...
	if (peer == chan) { /* pu->notquiteyet = 1 */
		...
		pu->notquiteyet = 0;
		ast_debug(6, "notquiteyet: %d\n", pu->notquiteyet);
	}
	if (peer) {
		char parkhint[80];
		snprintf(parkhint, sizeof(parkhint), "Local/%s@%s", exten, pu->parkinglot->cfg.parking_con);
		pbx_builtin_setvar_helper(peer, "PARKHINT", parkhint);
		ast_debug(6, "parkhint: %s\n", parkhint);
	}
	return 0;
}
	...
	if (tms > pu->parkingtime) {
		...
		if (pu->peername[0]) {
			...
			if (!ast_context_find_or_create(NULL, NULL, parking_con_dial, registrar)) {
				...
				ast_channel_lock(chan);
				pbx_builtin_setvar_helper(pu->chan, "_PARKEDCALLTIMEOUT", "");
...
static void manage_parkinglot(struct ast_parkinglot *curlot, const struct pollfd *pfds, int nfds, struct pollfd **new_pfds, int *new_nfds, int *ms)
{
	...
	AST_LIST_TRAVERSE_SAFE_BEGIN(&curlot->parkings, pu, list) {
		if (pu->notquiteyet || !pu->chan) { /* Pretend this one isn't here yet */
			ast_debug(6, "Value: %d\n", pu->notquiteyet);
			continue;
		}
